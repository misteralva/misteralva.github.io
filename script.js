...
const projectsData = [
    {
        ...
        github: "https://github.com/misteralva/Web-StarterKit",
        demo: null
    },
    {
        id: 1,
        ...
        features: [
            ...
            "Efectos parallax y reveal dinámicos",
            "Transiciones suaves entre secciones",
            "Optimización de rendimiento",
            "Responsive animations"
        ],
        technologies: ["GSAP 3.x", "ScrollTrigger", "JavaScript", "CSS3"],
        github: "https://github.com/misteralva/GSAP-Actividad",
        demo: null
    },
    {
        id: 2,
        ...
        features: [
            "Timeline animations complejas",
            "Morphing de elementos SVG",
            "Efectos de entrada y salida creativos",
            "Integración con eventos del usuario",
            "Código comentado y documentado"
        ],
        technologies: ["GSAP", "TextPlugin", "MorphSVG", "JavaScript", "HTML5"],
        github: "https://github.com/misteralva/GSAP-Practica",
        demo: null
    },
    {
        id: 3,
    ...
    },
    {
        ...
        features: [
            "Monitorización en tiempo real de iptables",
            "Análisis de logs y detección de anomalías",
            "Sistema de alertas por email/Slack",
            "Gráficas de tráfico y estadísticas",
    ...
    },
    {
        ...
        features: [
            ...
            "Scripts de monitorización de recursos",
            "Gestión de usuarios y permisos",
            "Updates y parches automatizados",
...
// ========== HERO TITLE — SPLIT CHAR ANIMATION ESPECTACULAR ==========
function splitAndAnimateTitle() {
    ...
    words.forEach((word, wIdx) => {
        ...
        // Espacio entre palabras (no en el último)
        if (wIdx < words.length - 1) {
            titleEl.appendChild(document.createTextNode(' '));
        }
    });

    // Timeline de entrada: chars caen en cascada con rotación 3D
    ...
    // Después de entrar, un pulso de glow sutil
    tl.to(allChars, {
...
// Hover interactivo en el título: cada letra brilla al pasar el mouse
...
// ========== SVG ANIMADO ==========
gsap.to('.svg-circle', {
    attr: { r: 200 }, duration: 3,
    repeat: -1, yoyo: true, ease: 'sine.inOut'
});

gsap.to('.svg-path', {
    attr: { d: 'M 100 300 Q 400 400 700 300 T 100 300' }, duration: 4,
    repeat: -1, yoyo: true, ease: 'sine.inOut'
});
    ...
    ScrollTrigger.create({
        ...
        onEnter: () => {
            gsap.to(stat, {
                ...
                snap: { textContent: 1 },
                onUpdate() { stat.textContent = Math.ceil(stat.textContent); }
            });
        }
    });
});
    ...
    items.forEach(item => {
        ...
        ScrollTrigger.create({
            ...
            onEnter: () => {
                ...
                if (percentEl) {
                    gsap.to({ val: 0 }, {
                        val: level, duration: 1.2, ease: 'power2.out',
                        onUpdate: function() { percentEl.textContent = Math.round(this.targets()[0].val) + '%'; }
                    });
                }
        ...
        item.addEventListener('mouseenter', () => {
            gsap.to(ring, { attr: { 'stroke-dashoffset': Math.max(0, targetOffset - circumference * 0.05) }, duration: 0.35, overwrite: 'auto' });
        });
        item.addEventListener('mouseleave', () => {
            gsap.to(ring, { attr: { 'stroke-dashoffset': targetOffset }, duration: 0.35, overwrite: 'auto' });
        });
    });
})();

// ========== CERTIFICACIONES — REVEAL CON EFECTO FLIP + STAGGER ==========
(function initCertReveal() {
    const cards = document.querySelectorAll('.cert-card');

    // Detectar filas para animar por fila
    // Cada card empieza invisible y rotada
    cards.forEach(card => {
        gsap.set(card, {
            opacity: 0,
            rotationY: 25,
    ...
    // Animación staggered por grupos de la grid
    ScrollTrigger.create({
...
// ========== PROYECTOS — REVEAL ESCALONADO CON CLIP-PATH ==========
(function initProjectReveal() {
    const cards = document.querySelectorAll('.project-card');

    cards.forEach((card, i) => {
        // Alternamos dirección izq/der para efecto cinematográfico
        const fromLeft = i % 2 === 0;
        gsap.set(card, {
            opacity: 0,
            x: fromLeft ? -60 : 60,
            y: 40,
            rotationZ: fromLeft ? -3 : 3,
            scale: 0.9,
        ...
        ScrollTrigger.create({
            ...
            onEnter: () => {
                gsap.to(card, {
                    ...
                    scale: 1,
                    duration: 0.8,
                    delay: (i % 3) * 0.12, // stagger dentro de la misma fila
                    ease: 'power3.out'
                });
            }
        });
...
filterBtns.forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        const interval = setInterval(() => {
            if (count < 5) {
                btnText.textContent = originalText.split('').map(() =>
                    '!<>-_\\/[]{}—=+*^?#'[Math.floor(Math.random() * 17)]
                ).join('');
                count++;
...
function openModal(projectId) {
    ...
    icon.className = project.tags[0] === 'Python' ? 'fab fa-python' :
                     project.tags[0] === 'GSAP' ? 'fas fa-magic' : 'fas fa-code';
...
function closeModal() {
    gsap.to('.modal-content', {
        ...
        onComplete: () => { modal.classList.remove('active'); document.body.style.overflow = 'auto'; }
    });
}
...
gsap.from('.timeline-item', {
    ...
    opacity: 0, x: -100, duration: 1, stagger: 0.3, ease: 'power3.out'
});

gsap.from('.timeline-dot', {
    scrollTrigger: { trigger: '.timeline', start: 'top 80%' },
    scale: 0, duration: 0.6, stagger: 0.3, ease: 'back.out(2)'
});
...
gsap.from('.social-link', {
    ...
    opacity: 0, scale: 0, duration: 0.5, stagger: 0.1, ease: 'back.out(2)'
});
...
document.querySelector('.contact-form').addEventListener('submit', e => {
    ...
    inputs.forEach(input => {
        if (!input.value.trim()) {
            isValid = false;
            gsap.to(input, { borderColor: '#ff0080', duration: 0.3 });
            gsap.fromTo(input, { x: -10 }, {
                x: 10, repeat: 3, yoyo: true, duration: 0.1,
                onComplete: () => gsap.to(input, { x: 0 })
            });
        } else {
            gsap.to(input, { borderColor: 'rgba(0,255,136,0.2)', duration: 0.3 });
        }
    });
    ...
    if (isValid) {
        ...
        gsap.to(submitBtn, {
            ...
            onComplete: () => {
                ...
                setTimeout(() => {
                    gsap.to(submitBtn, {
                        scale: 1, backgroundColor: 'transparent', color: 'var(--primary)', duration: 0.3,
                        onComplete: () => { submitBtn.innerHTML = originalHTML; document.querySelector('.contact-form').reset(); }
                    });
                }, 3000);
            }
        });
